...
public class TrafficLightBehaviourHandler {
    ...
    private static final double RED_STOP_GAP = 7.0;          // The calibrated gap to ensure the <=5 cm gap from the light
    private static final double SPEED_Scale_Per_Sec = 0.5;   // Speed per second scaling of about 0.5 cm/s
    private static final int RED_APPROACH_MAX_TIME = 4000;   // Max time the bot will move forward
    private static final int RED_APPROACH_MIN_TIME = 100;    // Min move time before the bot considers itself close enough to stop
    private static final int RED_POST_BEHAVIOUR_STOP  = 200; // Brief pause after stopping at the red light before resuming the redLightHandler() method

    //-------- Behaviour timing constants-------------//
    private static final int RED_WAIT_TIME = 1000;             // 1 sec wait time at a red light
    private static final int GREEN_PASS_DURATION = 2000;       // Duration for the green pass move
    private static final int GREEN_WAIT_TIME = 1000;           // Wait time after passing green
    private static final int BLUE_INITIAL_STOP_TIME = 1000;    // 1 sec Initial pause when first detected blue
    private static final int BLUE_BLINK_DELAY = 300;           // Delay for the blue underlight blink in the give-way sequence
    private static final int BLUE_TURN_TIME = 650;             // Duration for left and right turns
    private static final int BLUE_MOVE_FORWARD_TIME = 1000;    // Slow forward movement duration
    private static final int BLUE_FORWARD_PAUSE_TIME = 1000;   // 1 secPause after moving forward and stopping
    private static final int BLUE_REVERSE_TIME = 2000;         // Duration for reversing back
    private static final int BLUE_TURN_SPEED = 40;             // Turn speed for the blue light give-way sequence
     ...
     * @param greenPassSpeed the pass speed for the current driving mode.
     */
     ...
     * When UNKNOWN colour is detected sends a message and continues.
    ...
    private void stopAtRedLight() throws InterruptedException {
        ...
        double distanceToMove = ultrasoundDistance - RED_STOP_GAP;
        ...
        // if the time is large it gets capped to 4 secs hence an error detection
     ...
     * stop position, waits, then returns.
     ...
     * forward at the pass speed for a fixed duration (configured by driving mode), then stops briefly.
     * @throws InterruptedException if the thread is interrupted during the sequence.
     */
    private void greenLightHandler() throws InterruptedException {
        ...
        // small 1 sec wait after passing the green light before resuming
