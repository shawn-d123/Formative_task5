 ...
 * CS1814 Software Implementation — Traffic Light
...
public class TrafficLightMainController {
    ...
    /*
     * Volatile flags used as they are written by button-callback threads
     * and read by the main loop thread. hence ensures changes will
    ...
    private static final int CAUTIOUS_DETECTION_INTERVAL = 500;
    ...
    private static final int NORMAL_DETECTION_INTERVAL   = 400;
    ...
    private static final int FAST_DETECTION_INTERVAL     = 200;
        ...
        while (isSystemRunning) {
            ...
            if (distance > 0 && distance <= DETECTION_DISTANCE) {
                ...
                try {
                    ...
                    // requests termination is user terminates
                    if (terminate) {
                    ...
                    // Brief cooldown to prevent the same light being redetected immediately
                    Thread.sleep(REDETECTION_PAUSE);

                } finally {
                    // Resets flags to ensure that loop will continue to work even if an exception occurs
                    isHandlingLight = false;
                    enableRuntimeTermination(swiftBot);
                }
            }
    ...
    /**
     * Pauses until Button A or Button X is pressed on the SwiftBot during the start-up waiting screen.
     * Buttons B and Y are mapped to an invalid input screen rather.
    ...
    private static void waitForStartScreenInput(SwiftBotAPI swiftBot) throws InterruptedException {
        ...
        /*
         * Single-element arrays used as lambda callbacks cannot
         * write to  local variables, it acts as a final variable.
         */
    ...
    /**
     * Registers Button X as the universal runtime termination trigger.
     * Disables any existing handler first to prevent duplicate callbacks.
     * @param swiftBot takes the SwiftBot API instance.
     */
    ...
    /** Displays the invalid input screen for the startup screen when Button B or Y pressed. */
    ...
    /** Displays the invalid input screen for the termination screen when wrong button pressed. */
    ...
    /**
     * Displays the execution log screen with the run through information.
